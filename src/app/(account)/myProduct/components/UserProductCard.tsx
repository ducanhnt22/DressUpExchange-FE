...
export default function UserProductCard({ name, price, thumbnail, productId, token }: product) {
    ...
    <div>
      ...
      <div className="p-3 bg-slate-100 flex justify-around flex-col fill-transparent">
        ...
        <div className="flex justify-between mt-2">
          <button className="bg-orange-400 px-5 rounded-lg hover:bg-orange-600 text-white">Edit</button>
          <button className="bg-orange-400 py-2 px-3 rounded-lg hover:bg-orange-600 text-white" onClick={() => setOpen(true)}>
            Add Voucher
          </button>
        </div>
      </div>
      {open && <AddVoucherModal productId={productId} setOpen={setOpen} token={token} />}
    </div>
  );
}
